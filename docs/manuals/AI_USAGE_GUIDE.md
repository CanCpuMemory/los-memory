...
```bash
python3 -m memory_tool --profile claude observation add \
...
```bash
python3 -m memory_tool --profile claude memory timeline --limit 10
```

2. **Search for relevant context**:
```bash
python3 -m memory_tool --profile claude memory search "authentication"
python3 -m memory_tool --profile claude memory search "project:payments decision"
```

3. **Resume from checkpoint** (if available):
```bash
python3 -m memory_tool --profile claude checkpoint resume checkpoint-name
```
...
```bash
python3 -m memory_tool --profile claude checkpoint create \
...
```bash
python3 -m memory_tool --profile claude admin manage stats
```
...
python3 -m memory_tool --profile claude observation feedback \
  "修正: API密钥是yyy而非xxx" \
  --id 123
...
python3 -m memory_tool --profile claude tool log \
  --tool "search_files" \
  --input '{"query": "TODO"}' \
...
python3 -m memory_tool --profile claude tool stats
...
python3 -m memory_tool --profile claude tool suggest \
...
python3 -m memory_tool --profile claude observation link \
  --from 123 \
  --to 456 \
  --type refines
...
python3 -m memory_tool --profile claude observation related 123
...
# Remove a link
python3 -m memory_tool --profile claude observation unlink \
  --from 123 \
  --to 456
```
...
python3 -m memory_tool --profile shared observation add \
  --project "shared-docs" \
  --title "Project onboarding guide" \
  ...

# Retrieve from shared profile
python3 -m memory_tool --profile shared memory search "onboarding"
```
...
```bash
python3 -m memory_tool --profile claude observation add --title "Complex discussion" --summary "LLM hook summary" --raw "$(cat meeting.txt)" --auto-tags
```
...
```bash
# Create checkpoint
python3 -m memory_tool --profile claude checkpoint create --name "before-refactor" --tag "safe-point"

# List checkpoints
python3 -m memory_tool --profile claude checkpoint list

# Resume from checkpoint (restores project and shows recent observations)
python3 -m memory_tool --profile claude checkpoint resume 1
```
...
```bash
python3 -m memory_tool.viewer --profile claude
# Open http://localhost:8080

# With authentication
python3 -m memory_tool.viewer --profile claude --auth-token "my-secret"
...
python3 -m memory_tool --profile claude observation add --project "api-service" --kind "incident" \
...
python3 -m memory_tool --profile claude observation add --project "api-service" --kind "snippet" \
...
```bash
python3 -m memory_tool --profile claude observation add --project "data-pipeline" --kind "decision" \
...
```bash
python3 -m memory_tool --profile claude observation add --project "frontend" --kind "note" \
...
# Search
python3 -m memory_tool --profile claude memory search "query"

# Recent activity
python3 -m memory_tool --profile claude memory timeline

# Stats
python3 -m memory_tool --profile claude admin manage stats

# Export
python3 -m memory_tool --profile claude memory export --format json --output backup.json
```
